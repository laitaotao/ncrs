...
run('F:\soft\vlfeat-0.9.21\toolbox\vl_setup');
...
for iter = 1:100
    ...
    for s_i = seq_range
        ...
        for f_i = 1:num_frames-FrameGap
            ...
            % Random sampling.
            [ par{1} res{1} inx{1} tim{1} KKK] = mAGS(1,data,10000,model_type,ones(1, size(data,2)));% greedySampling(lim,data,max_NumHypoPerFrame,model_type);
            ...
            resTemp = ones(size(Data.ySparse,2), size(res{1},2));
            resTemp(visible_pts_ind,:) = res{1};
            Hypos.res = [Hypos.res  resTemp];
